...
> **原理说明**：`ALine` 组件采用**延迟求值**机制——它不会在挂载时立刻捕获插槽内容为静态 VNode，而是注册一个异步函数，等待游戏引擎执行到该行时才调用插槽函数。因此 `{{ nat }}` 和 `{{ dirty }}` 拿到的是 `Adv.check()` 返回后的正确值，而非挂载时的初始值。
...
在 CoC 模式下，可通过 `.hard` / `.extreme` 属性快速设定目标值：

```xml
<ACheck :target="() => Adv.status.FastTalk.hard" target-desc="话术（困难）">
...
- `'percent'`：**点数 ≤ 目标值** 为成功。适用于克苏鲁的呼唤等百面骰系统。
