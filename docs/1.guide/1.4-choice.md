...
| 参数名     | 类型                | 默认值       | 描述                                                                                          |
| ---------- | ------------------- | ------------ | --------------------------------------------------------------------------------------------- |
| `content`  | `string` 或 `VNode` | **必填**     | 选项按钮上显示的文字。可以是纯文本，也可以是一段 HTML 或 Vue 组件。                           |
...
| `check`    | `ADVUserCheck`      | `undefined`  | 检定的配置，检定会在 next 生效**之前**发生。                                                  |
| `onChoose` | `() => void`        | `undefined`  | 当该选项被选择时，触发该事件。                                                                |
